...
Author: Josh Nederveld, Glenn Welser
		...
		<form method="post" action="<?php echo $_SERVER['PHP_SELF'] . '?page=' . $slug; ?>">
			...
			<table class="form-table">
				<tr valign="top">
					...
					<td>
						<input type="text" id="header_images_home_class" name="header_images_home_class"
						       value="<?php echo get_option( 'header_images_home_class' ); ?>"/>
					</td>
				</tr>
				<tr valign="top">
					<th scope="row">
					...
					<td>
						<input type="text" id="header_images_interior_class" name="header_images_interior_class"
						       value="<?php echo get_option( 'header_images_interior_class' ); ?>"/>
					</td>
				</tr>
				<tr valign="top">
					<th scope="row">
					...
					<td>
						<input type="text" id="header_images_interior_secondary_class"
						       name="header_images_interior_secondary_class"
						       value="<?php echo get_option( 'header_images_interior_secondary_class' ); ?>"/>
					</td>
				</tr>
			</table>
...
function header_images_remove_url_protocol( $url ) {
...
// Set up home image. To be condensed later.
	if ( ! empty( $home_class ) ) {
		...
		if ( ! empty( $home_img_id ) ) {
			...
			// Mobile first!
			$output .= $selector . ' {background-image:url(' . header_images_remove_url_protocol($img_src_med[0]) . ')}';

			// Larger size for desktop.
			$output .= '@media (min-width: 900px) {' . $selector . ' {background-image:url(' . header_images_remove_url_protocol($img_src_large[0]) . ');}}';
		}
	...
	if ( ! empty( $interior_class ) ) {
		...
		if ( ! empty( $interior_img_id ) ) {
			...
			if ( ! empty( $interior_secondary_class ) ) {
				// For image circle.
				$output .= $selector . ' {background-image:url(' . header_images_remove_url_protocol($img_src_thumb[0]) . ');}';
			}
		}
	}
...
// Set up all other terms
	...
	foreach ( $terms as $key => $term ) {
		...
		if ( ! empty( $img ) ) {
			...
			// Mobile first!
			$output .= $selector . ' ' . $interior_class . ' {background-image:url(' . header_images_remove_url_protocol($img_src_med[0]) . ')}';

			// Larger size for desktop.
			$output .= '@media (min-width: 900px) {' . $selector . ' ' . $interior_class . ' {background-image:url(' . header_images_remove_url_protocol($img_src_large[0]) . ');}}';

			// For image circle.
			$output .= $selector . ' ' . $interior_secondary_class . ' {background-image:url(' . header_images_remove_url_protocol($img_src_thumb[0]) . ');}';
		}
	}
